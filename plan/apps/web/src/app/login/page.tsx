...
export default function LoginPage() {
  ...
  const handleCredentialsLogin = async (e: React.FormEvent) => {
    ...
    if (result?.error) {
      alert("Invalid credentials. Check your tab info.");
      setLoading(false);
    } else {
      router.push("/");
      router.refresh();
    ...
    <div className="min-h-screen flex items-center justify-center bg-pub-black bg-pub-texture bg-cover p-4 backdrop-blur-[2px]">
      <div className="bg-pub-charcoal/95 p-8 rounded-xl shadow-[0_0_20px_rgba(0,0,0,0.6)] w-full max-w-md border-2 border-pub-wood relative overflow-hidden">
        ...
        <form onSubmit={handleCredentialsLogin} className="flex flex-col gap-4">
          ...
          <button className="bg-pub-gold text-black py-3 rounded-lg font-extrabold hover:bg-white transition mt-4 shadow-lg border border-pub-gold uppercase tracking-wider">
            Log In
          </button>
        </form>

        <p className="mt-6 text-center text-sm text-gray-400 font-medium">
          No account? <Link href="/register" className="text-pub-gold font-bold hover:text-white hover:underline transition">Create Account</Link>
        </p>
      </div>
    </div>
  );
